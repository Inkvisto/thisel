...
impl SolidityHelper {
    ...
    pub fn get_styles(input: &str) -> Vec<SpannedStyle> {
        ...
        let comments_iter = comments.into_iter().map(|comment| {
            ...
            (loc.start(), Style::default().dim(), loc.end())
        });
...
impl Highlighter for SolidityHelper {
    ...
    fn highlight_char(&self, line: &str, pos: usize, _s: bool) -> bool {
        pos == line.len()
    }
    ...
    fn highlight_prompt<'b, 's: 'b, 'p: 'b>(
        ...
        if prompt.starts_with("(ID: ") {
            ...
            out.replace_range(
                id_span,
                &Self::paint_unchecked_owned(id, Color::Yellow.into()),
            );
            out.replace_range(
                1..=2,
                &Self::paint_unchecked_owned("ID", Color::Cyan.into()),
            );
        }
...
impl<'a> TokenStyle for Token<'a> {
    fn style(&self) -> Style {
        ...
        match self {
            ...
            | Modifier | Immutable | Unchecked => Color::Cyan.into(),
