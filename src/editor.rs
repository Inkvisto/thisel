...
use crate::app::App;
use crate::dispatcher::ChiselDispatcher;
...
impl Mode {
    pub fn block<'a>(&self) -> Block<'a> {
        let help = match self {
            ...
            Self::Suggest => "type Tab for next suggestion, type enter to choose, type Esc to back to normal mode",
    ...
    pub fn cursor_style(&self) -> Style {
        let color = match self {
            ...
            Self::Suggest => Color::LightMagenta,
...
// State of custom vim-like editor
pub struct Editor {
    pub mode: Mode,
...
impl Editor {
    ...
    pub fn change_mode(self, mode: Mode) -> Self {
        Self {
            mode,
            pending: Input::default(),
        }
        ...
        match self.mode {
            Mode::Normal | Mode::Visual | Mode::Operator(_) => {
                match input {
                    ...
                    Input {
                        key: Key::Enter, ..
                    } => app.submit_message(textarea.lines(), dispatcher).await,
            ...
            Mode::Suggest => match input {
                Input {
                    key: Key::Enter, ..
                } => {
                    app.choose_suggestion(textarea);
                    Transition::Mode(Mode::Insert)
                }
                Input { key: Key::Tab, .. } => {
                    app.iter_suggestion();
                    Transition::Mode(Mode::Suggest)
                }
                Input { key: Key::Esc, .. } => Transition::Mode(Mode::Insert),
            ...
            Mode::Insert => match input {
                ...
                Input { key: Key::Tab, .. } => {
                    app.highlight_current_suggestion();
                    Transition::Mode(Mode::Suggest)
                }
                input => {
                    textarea.input(input); // Use default key mappings in insert mode
                    app.suggest(textarea.lines().last().unwrap());
                    Transition::Mode(Mode::Insert)
                }
            },
        }
