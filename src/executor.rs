        ...
        let backend = match self.config.backend.take() {
            ...
            None => {
                let fork = self
                    .config
                    .evm_opts
                    .get_fork(&self.config.foundry_config, env.clone());
...
fn format_event_definition(event_definition: &pt::EventDefinition) -> Result<String> {
    let event_name = event_definition
        .name
        .as_ref()
...
impl Type {
    ...
    fn map_special(self) -> Self {
        if !matches!(
            self,
            Self::Function(_, _, _) | Self::Access(_, _) | Self::Custom(_)
        ) {
            return self;
        }
        ...
        this.map(Self::Builtin)
            .unwrap_or_else(|| match types.last().unwrap().as_str() {
        ...
        match self {
            ...
            Self::Custom(mut types) => {
                ...
                intermediate.and_then(|intermediate| {
                    Self::infer_custom_type(intermediate, &mut types, None)
                        .ok()
                        .flatten()
                })
            }
    ...
    fn get_function_return_type<'a>(
        ...
        let pt::Expression::Variable(contract_name) = intermediate
            .repl_contract_expressions
...
fn map_parameters(params: &[(pt::Loc, Option<pt::Parameter>)]) -> Vec<Option<Type>> {
    params
        ...
        .map(|(_, param)| {
            param
                .as_ref()
                .and_then(|param| Type::from_expression(&param.ty))
        })
        .collect()
}
...
fn types_to_parameters(
    ...
    types
        .into_iter()
        .filter_map(|ty| ty.and_then(|ty| ty.try_as_ethabi(intermediate)))
        .collect()
}
...
mod tests {
    ...
    fn test_global_vars() {
        ...
        let global_variables = {
            ...
            &[
                // abi
                (
                    "abi.decode(bytes, (uint8[13]))",
                    Tuple(vec![FixedArray(Box::new(Uint(8)), 13)]),
                ),
                (
                    "abi.decode(bytes, (address, bytes))",
                    Tuple(vec![Address, Bytes]),
                ),
                (
                    "abi.decode(bytes, (uint112, uint48))",
                    Tuple(vec![Uint(112), Uint(48)]),
                ),
            ...
            if !*is_preinstalled {
                ...
                match solc {
                    ...
                    Err(e) => {
                        ...
                        if solc
                            .map_err(SolcError::from)
                            .and_then(|solc| solc.version())
                            .is_ok()
                        {
    ...
    fn parse(s: &mut SessionSource, input: &str, clear: bool) -> IntermediateOutput {
        ...
        *s = _s.clone();
        ...
        s.generate_intermediate_output()
            .expect("could not generate intermediate output")
    }
    ...
    fn get_type(
        ...
        (
            Type::from_expression(&expr).map(Type::map_special),
            intermediate,
        )
    }
